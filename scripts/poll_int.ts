...
async function main() {
    ...
    // Hardcoded request id we just printed
    ...
    const status = await dmEvm.requestStatus(requestId);
    ...
    if (status === 2n) {
        // Find if any logs correspond to MessageReceived
        console.log("Status is completed! It worked! Checking logs for final payload...");
        const filter = dmEvm.filters.MessageReceived();
        const logs = await dmEvm.queryFilter(filter, -100); // look up last 100 blocks
